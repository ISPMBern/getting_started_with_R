...
The goals of this code and exercises are to...
...
To achieve those goals, you will work with data from Basel on Covid vaccinations to check data structure and missings. For data wrangling and exploration, you will work with data from the National Health and Nutrition Examination Survey (NHANES), a national survey that measures every few years health and nutrition of adults and children in the US, including survey data, health exams, and laboratory tests (<https://wwwn.cdc.gov/nchs/nhanes/>). Before we get started with importing the data, we have to prepare our environment.
...
## Set directories and folder structure
...
d_proj <- "C:/Users/cn23r745/OneDrive - Universitaet Bern/_Teaching/Introduction_to_R/Code/getting_started_with_R/exercises"
...
pacman::p_load(tidyverse, haven, table1, readxl, writexl, labelled, summarytools, utils, DT) 
...
Now that we are set-up with the R environment, we can start importing our data.
...
We will now import the NHANES dataset using the readxl-package . Before doing this, download the excel file "nhanes_for_R.xlsx" and save it to your folder "01_oridata".
...
# FALSE: R assigns default column names
...
After you imported your data, you should check your data structure to get an overview and to make sure that your data was imported correctly. There are different ways to do this:
...
-   **Data structure**: Provides overview of dataset structure, including variable types and the first few observations
    ...
    # option 3: This will open the dataset in a separate window with the first couple of hundred of rows
    # careful: The function "view" from the tibble package overlaps with tidyverse! So you have to specify the package
    tibble::view(nhanes)

    # option 4: To open the dataset in a separate window, you can use the utils and/or DT package
    utils::View(nhanes)
    DT::datatable(nhanes)
    ```
...
Download the excel file Covid19_vaccines_Basel and put it in your folder 01_oridata. This dataset has been altered and contains some information about the number of Covid-19 vaccinations in the Canton Basel-City between 2 January 2021 and July 1 2023. Find out about the following:
...
Let's say in `nhanes` , you want to:
...
-   **filter(RIAGENDR == 1) %\>%**\
    Filters the dataset where RIAGENDR == 1 is (male), and then passes this filtered dataset to the next function.
-   **summarize(mean_weight = mean(BMXWT, na.rm = TRUE))**\
    Calculates the mean of the `BMXWT` column and stores it as `mean_weight`. `na.rm = TRUE` tells R to ignore/remove missing values for this operation.
...
The select() function (from the dplyr-package) is used to pick specific columns from a dataset. This is particularly helpful when your dataset has many variables, but you need to focus on a few for your analysis. It can be used to
...
The `filter()` function (from the dplyr-package) is used to select rows in a dataset that meet certain conditions. It's a powerful tool for narrowing down data based on specific criteria. You use it to
...
-   **Sorting by multiple variables**: You can sort by multiple columns, specifying the order of precedence
    ...
    # Sort by age (ascending) and body weight (descending)
    dn <- df %>%
      arrange(RIDAGEYR, desc(BMXWT))
...
The `rename()` function (from dplyr-package) is used to rename variables in a dataset. It allows you to provide new, meaningful names to columns while preserving the dataset's structure. You can use it to

-   To improve readability / understanding of your dataset.
...
-   **Renaming variables based on patterns**: Add a prefix, suffix, or replace variable names that have some common patterns in their name.
    ...
    # Breakdown of the rename_with code
    # ~ sub: In general, this tells R to find pattern in a certain string and replace it with something you specify. The string pattern and replacement you specify in the following brackets.
    ...
    # starts_with: Indicates to look only for BMX at the start of the string.
...
-   **Categorizing variables using if_else**: Especially handy for binary categorizations. You can do more categories by nesting multiple if_else functions.
    ...
    df <- df %>%
      ...
      relocate(age_group_binary, .after = RIDAGEYR)
    head(df)
    ...
    df <- df %>%
        ...
        relocate(age_group_three, .after = age_group_binary)
...
1.  Convert smoker_status from numeric to a factor:
...
-   **Base R**
    ...
    sd(df$RIDAGEYR, na.rm = TRUE)   # Standard deviation of age
...
-   **tidyverse**
    ...
    # {.col} refers to the variable name (e.g., RIDAGEYR).
...
-   **Base R**
    ...
    # Frequency counts for BMI category
    table(df$BMI_category)
    table(df$BMI_category, useNA = "ifany") # also show missings

    # Proportions (relative frequencies)
    prop.table(table(df$BMI_category)) # ignores missings by default
    prop.table(table(df$BMI_category, useNA = "ifany")) # specify to includem issings
    ```
...
1.  Calculate basic descriptive statistics (minimum, mean, sd, median, and maximum) for the following numeric variables: `BPXSY1` (systolic blood pressure), `BPXDI1` (diastolic blood pressure).
...
ds %>% # specifying for each variable on its own
  summarize(
    ...
    SD_Systolic = sd(BPXSY1, na.rm = TRUE),
    ...
    SD_Diastolic = sd(BPXDI1, na.rm = TRUE),
...
-   **tidyverse**: In tidyverse, you can use the group_by() function to arrange data based upon a factor variable (e.g., age group), and to then calculate descriptives for each group.
    ...
    ## Optional: You can also add an analysis of variance to see if there are differences between the groups
    anova <- aov(BMI ~ age_group_three, data = df)
    summary(anova)

    ## For a t-test, the code looks like this
    ttest <- t.test(BMXWT ~ RIAGENDR, data = df, var.equal = TRUE)
    ttest
...
-   **table1**: You can also use table 1 to stratify calculations by certain groups, using the "\|" operator.
    ...
    # Summary table for BMI and BMXWT grouped by age_group
...
To assess associations between categorical variables, you need contingency tables or cross-tabulations. To do those, you can again work with Base R, tidyverse, and summarytools.
