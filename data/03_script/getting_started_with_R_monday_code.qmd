...
To achieve those goals, you will work data from the National Health and Nutrition Examination Survey (NHANES), a national survey that measures every few years health and nutrition of adults and children in the US, including survey data, health exams, and laboratory tests (<https://wwwn.cdc.gov/nchs/nhanes/>). Before we get started with importing the data, we have to prepare our environment.
...
-   **summarize(mean_weight = mean(BMXWT, na.rm = TRUE))**\
    Calculate the mean of the `Weight` column and store it as `mean_weight`. na.rm = TRUE tells R to ignore/remove missing values for this operation.
...
-   **distinct**: For finding duplicates
...
-   **fct_relevel**: For re-ordering levels
...
Before filtering, select the following variables: SEQN, RIAGENDR, SMQ040, BPXSY1, BPXDI1. Assign them to the object "ds".

For the following exercises, use the dataset "ds" and assign any operations to the object "dn".

1.  Filter rows where `RIAGENDR` is 2 (female).
2.  Filter rows where `BPXSY1` (systolic blood pressure) is greater than 120 and `BPXDI1` (diastolic blood pressure) is less than 80.
3.  Combine multiple conditions with `|`: Filter rows where `RIAGENDR` is 1 (male) OR `BPXSY1` is greater than 140.
...
# Pre-task: Select the variables SEQN, RIAGENDR, SMQ040, BPXSY1, BPXDI1
ds <- nhanes %>%
            select(SEQN, RIAGENDR, SMQ040, BPXSY1, BPXDI1)


# 1. Filter rows for people who smoke everday (SMQ040 is 1).
dn <- ds %>%
  filter(SMQ040 == 1)

# 2. Filter rows where BPXSY1 (systolic blood pressure) is greater than 120
...
-   **Sorting with missing values**: By default, arrange() places missing values (NA) at the end. You can handle missing values explicitly if needed
...
-   **Modifying existing variables**: In this example, change age (currently in years) to months. In this example, we overwrite the current age-variable. It is recommended to create a new variable
...
-   **Categorizing variables using case_when:** For categorizing variables, more flexibel for multiple conditions (\> 2).
...
We will use different packages for exploratory data analysis:
...
-   **tidyverse**
    ...
    df %>%
      count(RIAGENDR) %>%
      # calculates the proportion of each category relative to the total count of observations
...
### Exercise 12: Simple descriptive statistics
...
### Exercise 14: Descriptives by group
